...
# /Users/jeffreymiller/Dropbox/QTL_Paper/CODE/PREMAP/PLINK2RQTL.f2.R
...
ped = ped
map = map
out = paste(outdir, name, ".csvr", sep = "")
missing.genotype = "0"
no.fid = FALSE
no.parents = FALSE
no.sex = FALSE
...
PLINKtoCSVR <- function(ped = "test_complete.ped", map = "test.map", out = "cross.csvr", missing.genotype = "0", no.fid = FALSE, no.parents = FALSE, no.sex = FALSE, no.pheno = FALSE, verbose = FALSE){
  ...
  outCSVR <- rbind(
              c("ID", "", "", paste(peddata[,"FID"],peddata[,"IID"],sep="_")  ),
