...
### Previously determined that depth should be above 100 for a decent gt, and less than 150000
### find all bases that meet criteria
##$my_bamtools merge -list /home/jmiller1/QTL_Map_Raw/align/bamlist.txt |
##$my_bamtools filter -in stdin -mapQuality ">30" -isProperPair true |
##$my_samtools depth -m 50 -d 15000 /dev/stdin | gzip -c > /home/jmiller1/QTL_Map_Raw/bedfiles/depth.gz

##min=100
##max=15000
##zcat depth.txt | awk '$3>384' | awk '{OFS="\t"}{print $1,$2-1,$2,$3}' | $BT merge -i stdin -d 10 -c 4 -o mean,max |
##  awk '{OFS="\t"}{print $1,$2,$3,$4,$5,$3-$2}' | awk '$6>$min' | awk '$5<$max' > ${basedir}/metadata/radsites.bed
